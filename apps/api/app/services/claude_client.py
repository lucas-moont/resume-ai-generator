...
#
# NOTE: when ANTHROPIC_API_KEY is not set, requests are routed through the local `claude` CLI
# (headless `-p` print mode) instead of the `anthropic` SDK, so a logged-in Claude Code session
# can be used with no API key stored anywhere in this project (the SDK itself only ever reads
# ANTHROPIC_API_KEY / ANTHROPIC_AUTH_TOKEN — it cannot discover a local Claude Code session).
# The CLI manages its own thinking/output-token budget end-to-end, so CLAUDE_THINKING and
# CLAUDE_MAX_OUTPUT_TOKENS below apply only to the SDK path (i.e. only when ANTHROPIC_API_KEY
# is set) and have no equivalent knob on the CLI path.
...
def _run_claude_cli_sync(system: str, user: str, model_name: str) -> str:
...
async def _chat_json_claude_cli(system: str, user: str, model_name: str) -> str:
    return await anyio.to_thread.run_sync(_run_claude_cli_sync, system, user, model_name)
