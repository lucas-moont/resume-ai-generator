...
from app.services.github_client import fetch_user_repos, repos_to_context_string
...
from app.services.projects_loader import (
    ...
    projects_to_context_string,
)

app = FastAPI(title="Resume Agent API", version="0.1.0")
STREAM_LLM_TIMEOUT_SECONDS = 240
...
def _quality_issues(resume: ResumeDocument, job_description: str) -> list[str]:
    ...
    desired = [t for t in _QUALITY_SKILL_TERMS if t in jd]
    got = {_normalize_token(s) for s in resume.skills}
    ...
    for t in desired:
        ...
        options = aliases.get(norm, (norm,))
        if not any(opt in got for opt in options):
            missing.append(t)
    if missing:
        issues.append(f"Missing core job skills in technologies: {', '.join(missing[:6])}.")
...
Revise the resume to address issues without inventing facts. Return full JSON only."""
    ...
    improved = parse_resume_json(raw, profile, refine=True)
    return improved
...
Return JSON only."""
        try:
            extracted_raw = await chat_json(extraction_system, extraction_user, model=body.model or DEFAULT_OLLAMA_MODEL)
            ...
            raise HTTPException(
                status_code=502,
                detail=f"LLM error ({llm_backend_label()}) extracting Profile.pdf: {e}",
            ) from e
    ...
    local_ctx = projects_to_context_string(md_entries)
    gh_ctx = repos_to_context_string(repos)
    ...
    locale = body.locale or profile.locale or "pt-BR"

    user_msg = f"""Job description:
---
{body.job_description.strip()}
---

Profile JSON (canonical — do not invent beyond this):
{profile.model_dump_json(indent=2)}

{pdf_block}Unified projects context (markdown files merged with GitHub hints):
---
{projects_unified}
---

Local project files raw listing:
{local_ctx}
...
Return the tailored resume as JSON only."""
    ...
    try:
        ...
        raise HTTPException(status_code=502, detail=f"LLM error ({llm_backend_label()}): {e}") from e
    return resume
...
Return the tailored resume as JSON only."""
            ...
            yield _sse("error", {"message": f"LLM error ({llm_backend_label()}): {e}"})
...
async def export_pdf(body: PdfExportRequest):
    try:
        pdf_bytes = await render_resume_pdf(body.resume)
    except Exception as e:
