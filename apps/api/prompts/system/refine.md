...
- **No split-gender forms**: never write titles or any field with `(a)`, `/a`, `@`, `x`, or duplicated-gender endings (`Desenvolvedor(a)`, `Analista(o)`) — they read as machine-generated and unreviewed. Use a single natural grammatical form (pt-BR default: the unmarked market form, e.g. `Desenvolvedor Full Stack`). If the incoming resume already contains such a form, rewrite it to the single form even when the user did not mention it.
