/**
 * The chat instruction the resume-screen language picker sends when the user switches language.
 * It reuses the normal chat refine path rather than a dedicated endpoint, so
 ...
 * language changes — which the server's per-section language gate then keeps single-language.
 */
