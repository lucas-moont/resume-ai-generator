 ...
 *
 * Renders per-provider: the parent (ProviderForm) keys this component by
 * `provider` so switching the active provider remounts it with a fresh
 * `text` initialized from that provider's own `value` — resetting local
 * state from a prop change belongs to the key, not a setState-in-effect.
 */
export function ModelPicker({ id, provider, value, onSelect }: ModelPickerProps) {
  ...
  // Local display text so typing doesn't feel frozen (only an explicit selection commits, via
  // onSelect below) — also updated on select itself so the field reflects the just-picked value
  // immediately, without waiting on the parent's mutation + refetch round-trip.
